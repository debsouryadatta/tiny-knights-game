...
test('panning retains every cached terrain chunk still visible in the new viewport',()=>{
  let created=0;
  const g:any=new Proxy({getImageData(){return {data:new Uint8ClampedArray(0)};}},{get(target,key){return key in target?target[key]:()=>{};}});
  const previous=globalThis.document;
  globalThis.document={createElement(){created++;return {getContext(){return g;},addEventListener(){}};}} as any;
  try{
    const terrain=bakeTerrain({grass:{width:1,height:1}}),target={drawImage(){}};
    const paint=(left:number)=>{for(let i=0;i<40;i++)if(terrain.draw(target,{left,top:512,right:left+2047,bottom:2047}))return;assert.fail('Terrain never finished');};
    paint(0);
    const initial=created;
    paint(512);
    assert.equal(created-initial,3,'A one-column pan should bake only its three newly visible chunks');
  }finally{globalThis.document=previous;}
});
