...
export function createUI({ client, audio, onViewChange = () => {} }) {
  ...
  const leave=async()=>{try{if(client.leave)await client.leave();else client.disconnect();}catch(err){$('#lobby-error').textContent=String(err);return;}window.dispatchEvent(new Event("mobile-play-reset"));currentSession=null;$('#help-dialog').close();$('#results').close();$('#join-screen').hidden=false;$('#draft').hidden=false;$('#waiting-lobby').hidden=true;document.body.classList.add('join-active');joining=false;joiningButtons(false);updateJoinLabel();view.tactical=false;view.buildMode=false;sync();};
