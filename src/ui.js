...
export function createUI({ client, audio, companionNeedle, onViewChange = () => {} }) {
  ...
  <div class="hud"><div class="room-strip"><span class="status-dot"></span><span id="connection">Connecting</span> · <button id="copy-room" title="Copy duel invite link">Room <b id="room-code">—</b></button></div><div id="tactical-hint" class="tactical-hint" hidden>Realm overview · select a destination to move · M to return</div><section class="hero-panel"><div class="hero-title"><b id="hero-name">Your knight</b><small id="hero-class">Knight</small></div><div class="health" role="meter" aria-label="Hero health" aria-valuemin="0" aria-valuemax="100"><i id="hp-bar"></i></div><div class="hero-meta"><span id="hp-value">— HP</span><span id="kills">0 defeats</span></div></section><section class="companion"><div class="orders" id="orders" hidden>${[['gather_wood','Wood'],['gather_gold','Gold'],['escort','Escort'],['attack','Attack'],['defend','Defend']].map(([o,n])=>`<button data-order="${o}">${n}</button>`).join('')}</div><button class="companion-toggle" id="companion-toggle" aria-expanded="false"><span id="companion-name">Your companion</span> <span aria-hidden="true">⌃</span><small id="companion-state">Choose an order</small></button></section><div class="joystick" id="joystick" role="group" aria-label="Touch movement joystick"><i></i></div><div class="actions">${[['attack','⚔','Attack','Space'],['gather','♧','Gather','E'],['ability','✧','Ability','Q'],['build','♜','Tower','B'],['recall','⌂','Recall','R'],['map','◇','Map','M']].map(([a,icon,label,key])=>`<button data-action="${a}" title="${label}${a==='build'?' · select a nearby tile':''}"><kbd>${key}</kbd><strong aria-hidden="true">${icon}</strong><span>${label}</span></button>`).join('')}</div><button class="map-panel" id="map-button" aria-label="Toggle realm overview"><canvas id="minimap" width="180" height="180"></canvas><span>THE REALM <b>↗</b></span></button><div class="message" id="message" role="status" hidden></div></div>
  ...
  $('#orders').insertAdjacentHTML('beforeend',`<form id="companion-command" class="companion-command"><label for="companion-line">Tell your squire</label><div class="companion-command-row"><input id="companion-line" name="line" maxlength="120" autocomplete="off" placeholder="chop wood, get gold…"><button type="submit" id="companion-send">Say</button></div><small id="companion-needle-status">Needle loading…</small></form>`);
  ...
  listen($('#companion-command'),'submit',async e=>{
    ...
    try{
      ...
      if(result.ok){
        ...
        needleLine.value='';
        message(`Order: ${String(result.command.order).replaceAll('_',' ')}`);
      }else{
  ...
  const update=(state,session,status,error)=>{
    ...
    if(hero){const bank=state.bank[hero.team];$('#wood').textContent=Math.floor(bank.wood);$('#gold').textContent=Math.floor(bank.gold);$('#hero-name').textContent=hero.name;$('#hero-class').textContent=hero.hero;$('#hp-bar').style.width=`${Math.max(0,hero.hp/hero.maxHp*100)}%`;$('.health').setAttribute('aria-valuenow',String(Math.round(hero.hp/hero.maxHp*100)));$('#hp-value').textContent=hero.hp>0?`${Math.ceil(hero.hp)} / ${Math.ceil(hero.maxHp)} HP`:`Respawn ${Math.max(0,Math.ceil(hero.respawnAt-state.elapsed))}s`;$('#kills').textContent=`${hero.kills} defeats`;
      ...
      const companion=state.actors.find(a=>a.ownerId===hero.id&&a.kind==='companion');$('#companion-name').textContent=companion?.companion||'Companion';$('#companion-state').textContent=companion?`${String(companion.order).replaceAll('_',' ')} · ${Math.ceil(companion.hp)} HP`:'Returning to the realm';root.querySelectorAll('[data-order]').forEach(b=>b.classList.toggle('active',b.dataset.order===companion?.order));if(hero.lastAction&&hero.lastAction!==latestAction&&!view.buildMode){latestAction=hero.lastAction;message(hero.lastAction);}}
