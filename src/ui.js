...
export function createUI({ client, audio, onViewChange = () => {} }) {
  ...
  $('.join-heading').insertAdjacentHTML('beforeend','<div class="landing-meta"><span id="games-played" role="status">Games played · loading…</span><a href="/credits.html">Credits</a></div>');
  ...
  const enter=async mode=>{if(joining)return;const name=$('input[name="name"]').value.trim();if(!name){$('#join-error').textContent='Enter your name to play.';$('input[name="name"]').focus();return;}const room=roomInput.value.trim().toUpperCase();if(mode==='join'&&!room){$('#join-error').textContent='Enter your friend’s room code.';roomInput.focus();return;}joining=true;joiningButtons(true);setLoadState({});$('#join-error').textContent='';try{await client.join({name,size:1,mode,room:mode==='join'?room:undefined,hero:$('[data-choice="hero"] .selected').dataset.value,companion:$('[data-choice="companion"] .selected').dataset.value});}catch(err){$('#join-error').textContent=err?.message||'Could not connect. Please try again.';joining=false;joiningButtons(false);updateJoinLabel();}};
