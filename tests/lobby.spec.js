...
test('friend joins waiting room, readies, starts with host, and reconnects',async({browser,baseURL},info)=>{
 ...
 await host.goto('/');await host.locator('#create-room').click();await expect(host.locator('#waiting-lobby')).toBeVisible();
 const room=await host.locator('#lobby-copy').textContent();
 await guest.goto('/?room='+room);await guest.locator('input[name="name"]').fill('Friend');await guest.locator('#join').click();await expect(guest.locator('#waiting-lobby')).toBeVisible();
 ...
 const id=await guest.evaluate(()=>window.realm.state.playerId);await guest.reload();await guest.locator('#join').click();
