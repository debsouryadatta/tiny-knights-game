...
Companions use a deterministic planner by default. Optional client-side Needle 2 can map a typed line onto the same four orders (`gather`, `escort`, `attack`, `defend`) in the browser. Run `npm run fetch-needle` once to download the 14MB `public/needle2.cact` weights (gitignored). The match stays playable if the model is missing. No provider keys belong in client code.
