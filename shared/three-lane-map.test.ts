...
test('cached movement grid matches final terrain and rejects non-tile coordinates',async()=>{
  const {isLand,isRiver,isWall,isForest}=await import('./map');
  for(let y=0;y<HEIGHT;y++)for(let x=0;x<WIDTH;x++){
    assert.equal(isWalkable(x,y),isLand(x,y)&&(!isRiver(x,y)||isLane(x,y))&&!isWall(x,y)&&!isForest(x,y),`Terrain mismatch at ${x},${y}`);
  }
  for(const [x,y]of [[-1,8],[WIDTH,8],[8,-1],[8,HEIGHT],[8.5,8],[8,8.5],[NaN,8],[8,Infinity]])assert.equal(isWalkable(x,y),false);
});
