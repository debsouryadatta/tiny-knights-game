...
function terrainWalkable(x:number,y:number):boolean{return isLand(x,y)&&(!isRiver(x,y)||isLane(x,y))&&!isWall(x,y)&&!isForest(x,y);}
let walkable:Uint8Array|undefined;
export function isWalkable(x:number,y:number):boolean{
  if(!Number.isInteger(x)||!Number.isInteger(y)||x<0||x>=WIDTH||y<0||y>=HEIGHT)return false;
  return walkable ? walkable[y*WIDTH+x]===1 : terrainWalkable(x,y);
}
...
// Terrain is immutable after pocket thinning. Movement and route searches share
// this final grid rather than repeating lane and camp geometry for every step.
walkable=new Uint8Array(WIDTH*HEIGHT);
for(let y=0;y<HEIGHT;y++)for(let x=0;x<WIDTH;x++)walkable[y*WIDTH+x]=Number(terrainWalkable(x,y));
